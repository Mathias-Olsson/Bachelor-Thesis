...
%                                                                         %
% Script information: This script classifies the test experiments by      %
%                     Cosine distance based classifier, utilizing the     %
%                     filterbank data from the Filterbank.m script, csp   %
%                     data from the FilterbankCSP.m script and EOG data   %
%                     from the EOGAccuracy.m script                       %
%                                                                         %
    ...
    for trial = 1:testtrials
        ...
        % normalized channels exceed the subject specific limits
            ...
            elseif mean(difference) < rightlimit*optimalmultiplier
                prediction(trial) = 2;
            else % Classify by the trained Cosine model if the trial is regarded as EEG
                fextract_test_du = CSPFeatures(du(:,:),unkEval(3:mDR+2,:,trial),m_range);

                cosine_du = pdist2(features_du,fextract_test_du,'cosine');
                [~, cosine_index] = min(cosine_du);
                prediction(trial) = Y_du(cosine_index);
            end
        else
...
if BCICOMP == 0
    cd(append('C:\Users\Mathi\Documents\DTU\Bachelor Project\Experiments\',subject,'\Cosine'))
